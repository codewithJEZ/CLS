...
router.post('/', (req, res) => {
  ...
  const sql = `INSERT INTO buildings (name, code, description, is_featured) VALUES (?, ?, ?, ?)`;

  db.run(sql, [name, code, description || '', is_featured ? 1 : 0], function (err) {
    if (err) {
      console.error('POST /buildings error:', err.message);
      return res.status(500).json({ success: false, message: 'Failed to save building.' });
    }
    return res.status(201).json({ success: true, message: 'Building saved successfully.', id: this.lastID });
  });
});
...
router.put('/:id', (req, res) => {
  ...
  db.run(sql, [name, code, description || '', is_featured ? 1 : 0, id], function (err) {
    if (err) {
